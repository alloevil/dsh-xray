// Model layer: pure functions over collector output. No IO here.

const crypto = require('node:crypto');

/**
 * Replay patch layers over an empty row list, recording per-row provenance.
 *
 * Patch entry shapes (verified against dsh-base/web-app bundle patches and
 * dsh's loader semantics: id-targeted whole-config replacement, insert lists):
 *   - { insert: [row, ...] }          append new rows
 *   - { id, config?, disabled?, ... } override an existing row by id
 *
 * @param layers output of collectStatic().layers
 * @returns {{rows: Map<id, row>, orphans: [], provenance: Map<id, [event]>}}
 *   event: {layer, kind, file, line, action: 'insert'|'override', fields, values, before?}
 */
function replayLayers(layers) {
  const rows = new Map();
  const provenance = new Map();
  const orphans = []; // overrides targeting an id that does not exist (dsh warns, then skips)

  const record = (id, event) => {
    if (!provenance.has(id)) provenance.set(id, []);
    provenance.get(id).push(event);
  };

  for (const layer of layers) {
    layer.entries.forEach((entry, index) => {
      const meta = {
        layer: layer.name,
        kind: layer.kind,
        file: layer.file,
        line: layer.lines?.[index] ?? null,
      };
      if (entry && Array.isArray(entry.insert)) {
        for (const row of entry.insert) {
          const id = row.id ?? row.name;
          rows.set(id, { ...row, id });
          record(id, { ...meta, action: 'insert', fields: Object.keys(row), values: { ...row } });
        }
      } else if (entry && entry.id !== undefined) {
        const { id, ...rest } = entry;
        const current = rows.get(id);
        if (!current) {
          orphans.push({ ...meta, id, fields: Object.keys(rest) });
          return;
        }
        const before = {};
        for (const f of Object.keys(rest)) before[f] = current[f];
        // Loader semantics: whole-config replacement, not deep merge.
        rows.set(id, { ...current, ...rest });
        record(id, {
          ...meta,
          action: 'override',
          fields: Object.keys(rest),
          values: { ...rest },
          before,
        });
      }
    });
  }
  return { rows, provenance, orphans };
}

/** F1: per-row layer attribution table. */
function attribute(staticData) {
  const { rows, provenance, orphans } = replayLayers(staticData.layers);
  const table = [...rows.values()].map((row) => {
    const events = provenance.get(row.id) ?? [];
    const origin = events.find((e) => e.action === 'insert') ?? null;
    return {
      id: row.id,
      name: row.name ?? null,
      disabled: row.disabled === true,
      origin: origin
        ? { layer: origin.layer, kind: origin.kind, file: origin.file, line: origin.line }
        : null,
      overrides: events
        .filter((e) => e.action === 'override')
        .map((e) => ({
          layer: e.layer,
          kind: e.kind,
          file: e.file,
          line: e.line,
          fields: e.fields,
        })),
    };
  });
  return { schema: 'dsh-xray/attribute@1', rows: table, orphans, warnings: staticData.warnings };
}

/** F3: rows written by more than one layer, with the winning writer last. */
function conflicts(staticData) {
  const { rows, provenance } = replayLayers(staticData.layers);
  const out = [];
  for (const [id, events] of provenance) {
    // A conflict needs two writers touching the same field set beyond the insert.
    const writers = events.filter((e) => e.action === 'override');
    if (writers.length === 0) continue;
    const byField = new Map();
    for (const e of events) {
      for (const f of e.fields) {
        if (f === 'id') continue;
        if (!byField.has(f)) byField.set(f, []);
        byField.get(f).push({
          layer: e.layer,
          kind: e.kind,
          action: e.action,
          file: e.file,
          line: e.line,
          value: e.values?.[f],
        });
      }
    }
    const contested = [...byField.entries()].filter(([, w]) => w.length > 1);
    if (contested.length === 0) continue;
    out.push({
      id,
      fields: contested.map(([field, writers]) => ({
        field,
        writers,
        winner: writers[writers.length - 1].layer,
        effective: rows.get(id)?.[field],
      })),
    });
  }
  return {
    schema: 'dsh-xray/conflicts@1',
    semantics: 'per-field last-writer-wins, layers applied in load order',
    conflicts: out,
  };
}

/** F2: declared (static replay) vs actual (dump-config) diff. */
function diff(staticData, dumpData) {
  const declared = replayLayers(staticData.layers);
  const actualById = new Map(dumpData.rows.map((r) => [r.id, r]));

  const missingFromActual = []; // declared but dsh dropped it
  const missingFromDeclared = []; // in the boot tree but no static layer declares it
  const disabledMismatch = [];

  for (const [id, row] of declared.rows) {
    const actual = actualById.get(id);
    if (!actual) {
      missingFromActual.push({ id, name: row.name ?? null });
      continue;
    }
    if ((row.disabled === true) !== actual.disabled) {
      disabledMismatch.push({ id, declared: row.disabled === true, actual: actual.disabled });
    }
  }
  for (const row of dumpData.rows) {
    if (!declared.rows.has(row.id)) {
      missingFromDeclared.push({ id: row.id, name: row.name, provenance: row.provenance });
    }
  }

  // Installed out-of-tree plugins whose bundle patch rows never made it in.
  const inactivePackages = staticData.packages
    .filter((p) => p.dsh?.bundle)
    .filter(
      (p) =>
        ![...declared.rows.values()].some((r) => r.name === p.name) &&
        !dumpData.rows.some((r) => r.name === p.name),
    )
    .map((p) => ({ name: p.name, version: p.version }));

  return {
    schema: 'dsh-xray/diff@1',
    missingFromActual,
    missingFromDeclared,
    disabledMismatch,
    orphanOverrides: declared.orphans,
    inactivePackages,
  };
}

/** F9: content-addressed snapshot of the effective composition.
 *
 * @2 adds the two facts a lockfile needs to mean "my environment is
 * reproducible": per-package provenance (`source` + `integrity`, hashed by the
 * static collector) and, when a runtime snapshot is supplied, the services and
 * tool registrations observed live. Both runtime sections are `null` in
 * static-only mode — never an empty object, which would read as "none". */
function snapshot(staticData, dumpData, runtimeSnap = null) {
  const sha = (s) => crypto.createHash('sha256').update(s).digest('hex').slice(0, 16);
  const byName = (a, b) => (a.name < b.name ? -1 : a.name > b.name ? 1 : 0);
  const services = runtimeSnap
    ? (runtimeSnap.services ?? [])
        .map((s) => ({ name: s.name, provider: s.provider ?? null }))
        .sort(
          (a, b) =>
            byName(a, b) || (a.provider < b.provider ? -1 : a.provider > b.provider ? 1 : 0),
        )
    : null;
  const tools = runtimeSnap
    ? (runtimeSnap.tools ?? [])
        .map((t) => ({
          name: t.name,
          owner: runtimeSnap.toolOwners?.[t.name] ?? null,
          tokens: t.tokens,
        }))
        .sort(byName)
    : null;
  return {
    schema: 'dsh-xray/snapshot@2',
    createdAt: new Date().toISOString(),
    profile: staticData.profile,
    bundles: staticData.layers
      .filter((l) => l.kind === 'bundle')
      .map((l) => ({ name: l.name, version: l.version, patchHash: sha(l.text) })),
    patches: staticData.layers
      .filter((l) => l.kind !== 'bundle')
      .map((l) => ({ kind: l.kind, file: l.file, hash: sha(l.text) })),
    packages: staticData.packages.map((p) => ({
      name: p.name,
      version: p.version,
      source: p.source ?? null,
      integrity: p.integrity ?? null,
      files: p.files ?? null,
    })),
    services,
    tools,
    composedHash: dumpData ? sha(dumpData.raw) : null,
    rowCount: dumpData ? dumpData.rows.length : null,
  };
}

/** F4: service dependency graph from a runtime snapshot. */
function serviceGraph(snap) {
  const services = {}; // service name -> { providers: [], consumers: [] }
  const touch = (s) => (services[s] ??= { providers: [], consumers: [] });
  // Primary source: the reflect store (Impl records — authoritative provider
  // ownership). Callback `provide` fields are a fallback for older snapshots.
  for (const s of snap.services ?? []) {
    const node = touch(s.name);
    if (s.provider && !node.providers.includes(s.provider)) node.providers.push(s.provider);
  }
  const providerOf = new Map(); // plugin name -> provided services
  for (const p of snap.plugins) {
    for (const s of p.provide) {
      const node = touch(s);
      if (!node.providers.includes(p.name)) node.providers.push(p.name);
    }
    for (const s of p.inject) touch(s).consumers.push(p.name);
  }
  for (const [name, node] of Object.entries(services)) {
    for (const provider of node.providers) {
      if (!providerOf.has(provider)) providerOf.set(provider, []);
      providerOf.get(provider).push(name);
    }
  }
  // Cascade: disabling a provider of service S affects every consumer of S,
  // transitively through services those consumers themselves provide.
  const cascade = {};
  for (const [provider, provided] of providerOf) {
    const affected = new Set();
    const queue = [...provided];
    const seen = new Set(queue);
    while (queue.length) {
      const s = queue.shift();
      for (const consumer of services[s]?.consumers ?? []) {
        if (consumer === provider || affected.has(consumer)) continue;
        affected.add(consumer);
        for (const next of providerOf.get(consumer) ?? []) {
          if (!seen.has(next)) {
            seen.add(next);
            queue.push(next);
          }
        }
      }
    }
    if (affected.size) cascade[provider] = [...affected].sort();
  }
  const unsatisfied = [];
  for (const p of snap.plugins) {
    for (const s of p.inject) {
      if (!services[s] || services[s].providers.length === 0) {
        unsatisfied.push({ plugin: p.name, service: s });
      }
    }
  }
  return { schema: 'dsh-xray/deps@1', services, cascade, unsatisfied };
}

/** F5: unhealthy plugins from fiber states and transition history. */
function health(snap) {
  const healthy = [];
  const waiting = [];
  const unhealthy = [];
  for (const p of snap.plugins) {
    // FAILED = startup threw; a serialization error also counts. PENDING is
    // normal (waiting for services) but surfaces as "waiting" for visibility.
    const bad = p.fibers.filter((f) => f.error || f.state === 'FAILED');
    const pending = p.fibers.filter((f) => f.state === 'PENDING');
    const entry = {
      name: p.name,
      fibers: p.fibers.map((f) => ({ uid: f.uid, state: f.state, error: f.error })),
      transitions: snap.transitions?.[p.name] ?? [],
    };
    if (bad.length) unhealthy.push(entry);
    else if (pending.length) waiting.push(entry);
    else healthy.push(entry);
  }
  return {
    schema: 'dsh-xray/health@1',
    healthy: healthy.map((p) => p.name),
    waiting: waiting.map((p) => ({
      name: p.name,
      inject: snap.plugins.find((x) => x.name === p.name)?.inject ?? [],
    })),
    unhealthy,
    capturedAt: snap.capturedAt,
  };
}

/** F7: same-name registrations where a later writer silently shadows. */
function shadowing(snap) {
  // Service ownership comes from the reflect store; callback `provide` is a
  // secondary source. Cordis rejects same-layer duplicate tools itself, so
  // what we surface is cross-source duplication: one service claimed by
  // multiple plugins (isolation scopes make this legal — and invisible).
  const out = { schema: 'dsh-xray/shadow@1', services: [], registrars: [] };
  const providersByService = new Map();
  for (const s of snap.services ?? []) {
    if (!providersByService.has(s.name)) providersByService.set(s.name, []);
    if (s.provider) providersByService.get(s.name).push(s.provider);
  }
  for (const p of snap.plugins) {
    for (const s of p.provide) {
      const list = providersByService.get(s) ?? [];
      if (!list.includes(p.name)) providersByService.set(s, [...list, p.name]);
    }
  }
  for (const [service, providers] of providersByService) {
    if (providers.length > 1) out.services.push({ service, providers });
  }
  for (const p of snap.plugins) {
    const registrations = p.fibers
      .flatMap((f) => f.effects)
      .filter((e) => /tools\.register|commands?\./.test(e?.label ?? '')).length;
    if (registrations > 0) out.registrars.push({ plugin: p.name, registrations });
  }
  return out;
}

/** F8: estimated context cost — tool schemas plus prompt sections, each
 * attributed to the plugin that registered it (F9: owner join + per-plugin
 * rollup). Owners come from the snapshot's diff-based attribution tables;
 * a missing entry renders as null (`unattributed`), never a guess. */
function contextCost(snap) {
  const sectionOwners = snap.sectionOwners ?? {};
  const toolOwners = snap.toolOwners ?? {};
  const tools = (snap.tools ?? []).slice().sort((a, b) => b.tokens - a.tokens);
  const toolTokens = tools.reduce((sum, t) => sum + t.tokens, 0);
  const sections = (snap.promptAssembly?.sections ?? [])
    .slice()
    .sort((a, b) => b.tokens - a.tokens);
  const sectionTokens = sections.reduce((sum, s) => sum + s.tokens, 0);
  const total = toolTokens + sectionTokens;
  const share = (n) => (total ? Math.round((n / total) * 1000) / 10 : 0);

  // Per-plugin rollup: what does each plugin cost per request, and through
  // which entries? This is the context-budget view: sort by tokens, and the
  // top rows are the plugins silently taxing every request.
  const byOwner = new Map();
  const add = (owner, kind, name, tokens) => {
    const key = owner ?? 'unattributed';
    if (!byOwner.has(key))
      byOwner.set(key, { plugin: key, tokens: 0, sections: 0, tools: 0, entries: [] });
    const row = byOwner.get(key);
    row.tokens += tokens;
    row[kind] += 1;
    row.entries.push({ kind: kind === 'sections' ? 'section' : 'tool', name, tokens });
  };
  for (const s of sections) add(sectionOwners[s.name], 'sections', s.name, s.tokens);
  for (const t of tools) add(toolOwners[t.name], 'tools', t.name, t.tokens);
  const owners = [...byOwner.values()]
    .sort((a, b) => b.tokens - a.tokens)
    .map((row) => ({
      ...row,
      share: share(row.tokens),
      entries: row.entries.sort((a, b) => b.tokens - a.tokens),
    }));

  return {
    schema: 'dsh-xray/cost@1',
    totalTokens: total,
    toolTokens,
    sectionTokens,
    toolCount: tools.length,
    sectionCount: sections.length,
    tools: tools.map((t) => ({
      name: t.name,
      tokens: t.tokens,
      share: share(t.tokens),
      owner: toolOwners[t.name] ?? null,
    })),
    sections: sections.map((s) => ({
      name: s.name,
      tokens: s.tokens,
      share: share(s.tokens),
      owner: sectionOwners[s.name] ?? null,
    })),
    owners,
    promptObservedAt: snap.promptAssembly?.at ?? null,
    capturedAt: snap.capturedAt,
  };
}

/** F10: skill context cost — the catalog line every request carries per
 * skill, plus each skill's on-load body size. Read-only companion to the
 * ecosystem's skill managers: xray prices, it never toggles. */
function skillCost(snap) {
  const observed = snap.skillCatalog;
  const schema = 'dsh-xray/skills@1';
  if (!observed) {
    return { schema, available: false, skills: [], totals: null, capturedAt: snap.capturedAt };
  }
  const rows = observed.skills
    .slice()
    .sort((a, b) => b.catalogTokens - a.catalogTokens)
    .map((s) => ({
      name: s.name,
      provider: s.provider,
      source: s.source,
      modelInvocable: s.modelInvocable,
      catalogTokens: s.catalogTokens,
      bodyTokens: s.bodyTokens,
    }));
  const invocable = rows.filter((s) => s.modelInvocable);
  const catalogEntryTokens = invocable.reduce((sum, s) => sum + s.catalogTokens, 0);
  return {
    schema,
    available: true,
    skills: rows,
    totals: {
      count: rows.length,
      invocable: invocable.length,
      // Only model-invocable skills enter the durable catalog; its framing
      // is a fixed one-off on top of the per-skill lines.
      catalogEntryTokens,
      catalogOverheadTokens: observed.catalogOverheadTokens,
      residentTokens:
        invocable.length > 0 ? catalogEntryTokens + observed.catalogOverheadTokens : 0,
    },
    observedAt: observed.capturedAt,
    capturedAt: snap.capturedAt,
  };
}

/** F11: per-request ledger — one classified bill per LLM call, newest
 * first, with prefix-stability and delta markers. The time axis the static
 * cost view lacks. */
function requestLedger(snap) {
  const observed = snap.requestLedger;
  const schema = 'dsh-xray/requests@1';
  if (!observed || Object.keys(observed).length === 0)
    return { schema, available: false, sessions: [], capturedAt: snap.capturedAt };
  const sessions = Object.entries(observed)
    .map(([sessionId, entries]) => ({
      sessionId,
      lastAt: entries[entries.length - 1]?.at ?? 0,
      requests: entries
        .slice()
        .reverse()
        .map((e, i, arr) => ({
          seq: entries.length - i,
          at: e.at,
          purpose: e.purpose,
          provider: e.provider,
          model: e.model,
          total: e.total,
          system: e.system,
          toolSchemas: e.toolSchemas,
          history: e.history,
          toolResults: e.toolResults,
          toolResultRows: e.toolResultRows,
          historyMessages: e.historyMessages,
          prefixStable: e.prefixStable,
          deltaTotal: e.deltaTotal,
        })),
    }))
    .sort((a, b) => b.lastAt - a.lastAt);
  return { schema, available: true, sessions, capturedAt: snap.capturedAt };
}

/** Registry names are plugin callback names (PluginTimer); rows carry the
 * loader names (plugin-timer). Normalized comparison bridges the two — a
 * heuristic, so runtime-only names are reported, never treated as errors. */
function normalizeName(s) {
  return String(s ?? '')
    .toLowerCase()
    .replace(/[^a-z0-9]/g, '');
}

/** F12: reconcile the declared (static) composition against the runtime registry. */
function verify(staticData, snap, { staticMtimeMs = null } = {}) {
  const { rows } = replayLayers(staticData.layers);
  const norm = normalizeName;
  const runtimeByNorm = new Map();
  for (const p of snap.plugins) {
    if (p.name) runtimeByNorm.set(norm(p.name), p.name);
  }
  const matchedNorms = new Set();
  const matched = [];
  const declaredNotRunning = [];
  const disabledButRunning = [];
  const declaredByNorm = new Map(); // norm -> {id, name, disabled}
  for (const row of rows.values()) {
    for (const key of [norm(row.name), norm(row.id)]) {
      if (key)
        declaredByNorm.set(key, {
          id: row.id,
          name: row.name ?? null,
          disabled: row.disabled === true,
        });
    }
    const candidates = [norm(row.name), norm(row.id)].filter(Boolean);
    const key = candidates.find((k) => runtimeByNorm.has(k)) ?? null;
    if (key) matchedNorms.add(key);
    const entry = { id: row.id, name: row.name ?? null };
    if (row.disabled === true) {
      if (key) disabledButRunning.push({ ...entry, runtime: runtimeByNorm.get(key) });
    } else if (key) {
      matched.push({ ...entry, runtime: runtimeByNorm.get(key) });
    } else {
      declaredNotRunning.push(entry);
    }
  }
  const undeclaredRuntime = [...runtimeByNorm.entries()]
    .filter(([k]) => !matchedNorms.has(k))
    .map(([, name]) => name)
    .sort();
  const capturedMs = Date.parse(snap.capturedAt ?? '');
  const stale =
    staticMtimeMs != null && Number.isFinite(capturedMs) ? staticMtimeMs > capturedMs : null;

  // services / tools against the declared rows. The static side carries ids,
  // names, config and disabled — nothing else: no bundle patch and no
  // `package.json#dsh` field declares a provide or a tool list (checked
  // against the installed @deepseek-ai bundles, 2026-09-16), so these
  // sections reconcile *identity* (is the runtime provider/owner a declared,
  // enabled row?) rather than declaration equality. Reported, not guessed.
  const declaredRowFor = (name) => (name ? (declaredByNorm.get(norm(name)) ?? null) : null);
  const services = (snap.services ?? []).map((s) => ({
    name: s.name,
    provider: s.provider ?? null,
    row: declaredRowFor(s.provider),
  }));
  const toolOwners = snap.toolOwners ?? {};
  const tools = Object.keys(toolOwners)
    .map((name) => ({ name, owner: toolOwners[name] ?? null }))
    .sort((a, b) => (a.name < b.name ? -1 : a.name > b.name ? 1 : 0))
    .map((t) => ({ ...t, row: declaredRowFor(t.owner) }));
  const unattributed = (snap.tools ?? [])
    .map((t) => t.name)
    .filter((name) => !(name in toolOwners))
    .sort();

  return {
    schema: 'dsh-xray/verify@2',
    capturedAt: snap.capturedAt ?? null,
    stale,
    matched,
    declaredNotRunning,
    disabledButRunning,
    undeclaredRuntime,
    unsatisfied: serviceGraph(snap).unsatisfied,
    services: {
      checked: services.length,
      rows: services,
      providerUndeclared: services
        .filter((s) => s.provider && !s.row)
        .map((s) => ({ name: s.name, provider: s.provider })),
      providerDisabled: services
        .filter((s) => s.row?.disabled)
        .map((s) => ({ name: s.name, provider: s.provider, id: s.row.id })),
    },
    tools: {
      checked: tools.length,
      rows: tools,
      ownerUndeclared: tools
        .filter((t) => t.owner && !t.row)
        .map((t) => ({ name: t.name, owner: t.owner })),
      ownerDisabled: tools
        .filter((t) => t.row?.disabled)
        .map((t) => ({ name: t.name, owner: t.owner, id: t.row.id })),
      unattributed,
    },
    notes: [
      'static layers declare id/name/config/disabled only: neither bundle patches nor package.json#dsh carry provide or tool lists, so services and tools are reconciled against the declared row set, not against a declaration',
    ],
  };
}

/** F13: provenance chain for one tool — who registered it, what that plugin
 * injects, who provides those services, and on up to the plugins that inject
 * nothing (the kernel bundle roots). Read from the runtime snapshot alone;
 * every row names its source field. */
function whyTool(snap, name) {
  const toolOwners = snap.toolOwners ?? {};
  const tool = (snap.tools ?? []).find((t) => t.name === name) ?? null;
  const owner = toolOwners[name] ?? null;
  const plugins = new Map();
  for (const p of snap.plugins ?? []) if (p.name) plugins.set(p.name, p);
  const byNorm = new Map();
  for (const p of snap.plugins ?? []) if (p.name) byNorm.set(normalizeName(p.name), p.name);

  const providersOf = (service) => {
    const out = [];
    for (const s of snap.services ?? []) {
      if (s.name === service && s.provider && !out.includes(s.provider)) out.push(s.provider);
    }
    for (const p of snap.plugins ?? []) {
      if ((p.provide ?? []).includes(service) && !out.includes(p.name)) out.push(p.name);
    }
    return out;
  };

  const rows = [{ depth: 0, kind: 'tool', name, owner, tokens: tool?.tokens ?? null }];
  const visited = new Set();
  const expand = (pluginName, depth) => {
    const plugin = plugins.get(pluginName) ?? plugins.get(byNorm.get(normalizeName(pluginName)));
    if (!plugin) {
      rows.push({ depth, kind: 'plugin', name: pluginName, injects: [], unresolved: true });
      return;
    }
    if (visited.has(plugin.name)) {
      rows.push({
        depth,
        kind: 'plugin',
        name: plugin.name,
        injects: plugin.inject ?? [],
        revisited: true,
      });
      return;
    }
    visited.add(plugin.name);
    const injects = plugin.inject ?? [];
    rows.push({
      depth,
      kind: 'plugin',
      name: plugin.name,
      injects,
      root: injects.length === 0,
    });
    for (const service of injects) {
      const providers = providersOf(service);
      rows.push({ depth: depth + 1, kind: 'service', name: service, providers });
      for (const provider of providers) expand(provider, depth + 2);
    }
  };
  if (owner) expand(owner, 1);

  return {
    schema: 'dsh-xray/why@1',
    query: name,
    found: Boolean(owner),
    tool: { name, owner, tokens: tool?.tokens ?? null },
    rows,
  };
}

function contextSlo(snap, policy = {}) {
  const cost = contextCost(snap);
  const thresholds = { totalTokens: 16000, toolTokens: 2000, unattributed: 0, ...(policy.thresholds ?? {}) };
  const checks = [
    { id:'totalTokens', label:'request context', value:cost.totalTokens, limit:thresholds.totalTokens, unit:'tokens' },
    { id:'toolTokens', label:'tool schemas', value:cost.toolTokens, limit:thresholds.toolTokens, unit:'tokens' },
    { id:'unattributed', label:'unattributed entries', value:(cost.sections ?? []).filter((s) => !s.owner).length + (cost.tools ?? []).filter((t) => !t.owner).length, limit:thresholds.unattributed, unit:'entries' },
  ].map((c) => ({ ...c, status:c.value > c.limit ? 'fail' : c.value > c.limit * .9 ? 'warn' : 'pass' }));
  return { schema:'dsh-xray/slo@1', thresholds, status:checks.some((c)=>c.status==='fail')?'fail':checks.some((c)=>c.status==='warn')?'warn':'pass', checks };
}

function compositionDiff(current, baseline) {
  if (!baseline) return { schema:'dsh-xray/diff@2', baseline:null, status:'no-baseline', changes:[] };
  const map = (a, key) => new Map((a ?? []).map((x) => [x[key], x]));
  const before = map(baseline.plugins, 'name'); const after = map(current.plugins, 'name');
  const changes = [];
  for (const name of after.keys()) if (!before.has(name)) changes.push({ kind:'added', name });
  for (const name of before.keys()) if (!after.has(name)) changes.push({ kind:'removed', name });
  const oldTools = new Map((baseline.tools ?? []).map((x)=>[x.name,x.tokens]));
  for (const t of current.tools ?? []) if (oldTools.has(t.name) && oldTools.get(t.name)!==t.tokens) changes.push({kind:'tool-token-drift',name:t.name,before:oldTools.get(t.name),after:t.tokens,delta:t.tokens-oldTools.get(t.name)});
  return { schema:'dsh-xray/diff@2', baseline:'provided', status:changes.length?'changed':'clean', changes };
}

function impactAssessment(snap, plugin) {
  const graph = serviceGraph(snap); const owned = Object.entries(snap.toolOwners ?? {}).filter(([,owner])=>owner===plugin).map(([name])=>name);
  const provided = Object.entries(graph.services).filter(([,n])=>n.providers.includes(plugin));
  const consumers = [...new Set(provided.flatMap(([,n])=>n.consumers))].filter((x)=>x!==plugin);
  return { schema:'dsh-xray/impact@1', plugin, evidence:'runtime snapshot', ownedTools:owned, providedServices:provided.map(([name])=>name), directConsumers:consumers, risk:consumers.length?'review':'low-observed-impact', recommendation:'inspect and generate a patch preview; no changes were applied' };
}

function evidenceGraph(snap) {
  const cost = contextCost(snap); const graph = serviceGraph(snap);
  const nodes = []; const edges = [];
  for (const p of snap.plugins ?? []) nodes.push({ id:`plugin:${p.name}`, kind:'plugin', label:p.name, trust:'verified' });
  for (const s of cost.sections ?? []) { nodes.push({id:`section:${s.name}`,kind:'section',label:s.name,tokens:s.tokens,trust:'estimated'}); if(s.owner) edges.push({from:`plugin:${s.owner}`,to:`section:${s.name}`,relation:'contributes'}); }
  for (const t of cost.tools ?? []) { nodes.push({id:`tool:${t.name}`,kind:'tool',label:t.name,tokens:t.tokens,trust:'estimated'}); if(t.owner) edges.push({from:`plugin:${t.owner}`,to:`tool:${t.name}`,relation:'registers'}); }
  for (const [name, service] of Object.entries(graph.services)) { nodes.push({id:`service:${name}`,kind:'service',label:name,trust:'verified'}); for(const p of service.providers) edges.push({from:`plugin:${p}`,to:`service:${name}`,relation:'provides'}); for(const c of service.consumers) edges.push({from:`service:${name}`,to:`plugin:${c}`,relation:'consumed-by'}); }
  const attributable = [...(cost.sections ?? []), ...(cost.tools ?? [])].filter((x)=>x.owner).length;
  const total = (cost.sections?.length ?? 0) + (cost.tools?.length ?? 0);
  return { schema:'dsh-xray/evidence-graph@1', nodes, edges, coverage:{attributed:attributable,total,percent:total?Math.round(attributable/total*100):100} };
}

module.exports = {
  evidenceGraph,
  contextSlo,
  compositionDiff,
  impactAssessment,
  replayLayers,
  normalizeName,
  attribute,
  conflicts,
  diff,
  snapshot,
  serviceGraph,
  health,
  shadowing,
  contextCost,
  skillCost,
  requestLedger,
  verify,
  whyTool,
};
