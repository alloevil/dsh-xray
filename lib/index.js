            ...
            wctx.webServer,
            {
              summary: () => {
                ...
                return {
                  ...
                  waiting: health(snap).waiting.length,
                  healthy: health(snap).healthy.length,
                  services: Object.keys(serviceGraph(snap).services).length,
                  contextTokens: contextCost(snap).totalTokens,
              ...
              slo: () => contextSlo(freshSnap()),
              impact: (params) => impactAssessment(freshSnap(), params?.get('plugin') ?? ''),
              diff: () => compositionDiff(freshSnap(), null),
              graph: () => evidenceGraph(freshSnap()),
            },
    ...
    apply: (tctx) => {
      ...
      tctx.tools.register(
        defineTool({
          ...
          description:
            'Introspect the live plugin composition of this harness. View list: summary | deps | health | cost | shadow | skills | requests | slo | graph | diff | impact (default summary). This exposes context cost, evidence graph, composition drift, and read-only impact; the panel is served at /xray.',
          parameters: {
            view: {
              type: 'string',
              description: 'summary | deps | health | cost | shadow | skills | requests | slo | graph | diff | impact (default summary)',
            },
            plugin: { type: 'string', description: 'Plugin name for impact view' },
          },
          output: {
          ...
          async execute(args) {
            ...
            if (args.view === 'slo') return contextSlo(snap);
            if (args.view === 'graph') return evidenceGraph(snap);
            if (args.view === 'diff') return compositionDiff(snap, null);
            if (args.view === 'impact') return impactAssessment(snap, args.plugin ?? '');
            return {
              schema: 'dsh-xray/summary@1',
              plugins: snap.plugins.length,
              unhealthy: health(snap).unhealthy.length,
