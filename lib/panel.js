...
<style>
  ...
  *{box-sizing:border-box;margin:0}
  ...
  .panel-head{display:flex;justify-content:space-between;gap:16px;align-items:baseline;margin-bottom:12px}.panel-title{font-size:12px;color:var(--muted);letter-spacing:.16em;text-transform:uppercase}.panel-note{color:var(--faint);font-size:10px}
  .metric-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-bottom:14px}.metric{padding:13px 14px;background:var(--panel2);border:1px solid var(--border);border-radius:10px}.metric-label{font-size:10px;color:var(--muted)}.metric-value{font-size:24px;font-weight:700;margin-top:6px}.metric-value.ok{color:var(--green)}.metric-value.warn{color:var(--red)}
  .waterfall{display:grid;gap:9px}.water-row{display:grid;grid-template-columns:150px 1fr 80px;gap:12px;align-items:center}.water-label{color:var(--muted)}.water-track{height:10px;background:#1b2636;border-radius:99px;overflow:hidden}.water-fill{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,var(--blue),var(--cyan));transition:width .5s}.water-value{text-align:right;color:var(--text)}
...
<main class="xray-shell">
  <header class="xray-top">
    ...
    <div class="top-meta"><span class="live-dot"></span><span id="freshness">live</span><br><span>DeepSeek Harness</span></div>
  </header>
...
const TRUST = { summary:'verified · live snapshot', health:'verified · lifecycle state', deps:'verified · runtime graph', cost:'estimated · ~4 chars/token', shadow:'verified · last writer wins', skills:'estimated · pricing only', requests:'verified · ledger counts' };
...
function openEntry(kind, name) {
  ...
  overlay.querySelector('.entry-close').onclick = close;
...
const renderers = {
  ...
  deps(d) {
    ...
    if (d.unsatisfied.length) html = '<p class="warn" title="' + escAttr(TIPS.wants) + '">' + d.unsatisfied.length + ' unsatisfied inject(s) — these plugins wait forever unless a provider is added</p>' + html;
    return html;
  },
  cost(d) {
    ...
    for (const session of d.sessions) {
      ...
      for (const r of session.requests) {
        ...
        html += '<div class="request-card"><div class="request-head"><span>#' + r.seq + ' · ' + esc(r.purpose ?? 'chat') + '</span><span>~' + r.total + ' tokens · <b class="' + deltaClass + '">Δ ' + delta + '</b></span></div><div class="request-bars">'
          + [['system', r.system], ['tools', r.toolSchemas], ['history', r.history], ['results', r.toolResults]].map(([label, value]) => '<div class="request-bar"><span>' + label + '</span><i style="width:' + Math.max(2, value / total * 100) + '%"></i><span class="num">' + value + '</span></div>').join('')
